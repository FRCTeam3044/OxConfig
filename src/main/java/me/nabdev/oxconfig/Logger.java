...
class Logger {
    ...
    private static boolean shouldLog(LogMessage message){
        if(OxConfig.loggingMode == OxConfig.LoggingMode.None) return false;
        if(OxConfig.loggingMode == OxConfig.LoggingMode.Info) return true;
        if(OxConfig.loggingMode == OxConfig.LoggingMode.Warnings && message.type != MessageType.Info) return true;
